...
fn main() -> io::Result<()> {
    ...
    let mut packets = Vec::<PacketNode>::new();

    let dividers = [
        PacketNode::List(vec![PacketNode::List(vec![PacketNode::Integer(2)])]),
        PacketNode::List(vec![PacketNode::List(vec![PacketNode::Integer(6)])]),
    ];
    ...
    for line in stdin.lines() {
        ...
        packets.push(PacketNode::from_string(line).unwrap());
    }

    packets.append(&mut dividers.to_vec());
    packets.sort();

    let prod = (packets.iter().position(|el| el == &dividers[0]).unwrap() + 1) * (packets.iter().position(|el| el == &dividers[1]).unwrap() + 1);
    println!("{}", prod);
