...
fn main() -> io::Result<()> {
    ...
    let occupied = tree.get_node(0).unwrap().get_size();
    let needed = 30000000 - (70000000 - occupied);
    let mut min_size = 70000000;
    tree.iter().filter(|x| x.is_dir()).filter(|x| x.get_size() >= needed).for_each(|x| min_size = min_size.min(x.get_size()));
